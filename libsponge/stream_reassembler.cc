...
StreamReassembler::StreamReassembler(const size_t capacity)
    : _output(capacity)
    , _capacity(capacity)
    , _unassembled_buffer(capacity, ' ')
    , _occupied(capacity, false)
    , _seperator(0)
    , _first_unassembled_index(0)
    , _unassembled_bytes(0)
    , _is_eof_data_received(false) {}
...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    // write reassembled data to output byte stream
    write_to_output();

    if (_is_eof_data_received &&
        (_unassembled_bytes == 0)) {  // if all data are written into the output stream in right order,
...
size_t StreamReassembler::store_data(const string &data, const size_t index) {
    ...
    size_t bytes_can_be_stored =
        std::min(data.size(), _capacity - bytes_reassembled_but_not_read - index + _first_unassembled_index);
...
void StreamReassembler::write_to_output() {
    std::stringstream reassembled;  // temp stream for reassembled data
    size_t count = 0;               // counter for reassembled bytes

    // transfer reassembled data from buffer to temp stream
    size_t buf_i = (count + _seperator) % _capacity;  // index of the buffer
    while (_occupied[buf_i]) {
        reassembled << _unassembled_buffer[buf_i];
        _unassembled_buffer[buf_i] = ' ';
        _occupied[buf_i] = false;
        _unassembled_bytes--;

        count++;
        buf_i = (count + _seperator) % _capacity;
    }

    // write them to output byte stream
    _output.write(reassembled.str());

    // update member variables
    _first_unassembled_index += count;
    _seperator = (_seperator + count) % _capacity;
}